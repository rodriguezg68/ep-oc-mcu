...
BME680::BME680(mbed::I2C* i2c, uint8_t adr) {
	bme680_i2c = i2c;
...
bool BME680::performReading(void) {
    ...
    debug("Set settings, result %d \r\n", result);
    if (result != BME680_OK)
        return false;

    /* Set the power mode */
    result = bme680_set_sensor_mode(&gas_sensor);
    debug("Set power mode, result %d \r\n", result);
    if (result != BME680_OK)
        return false;
    ...
    debug("Get sensor data, result %d \r\n", result);
    if (result != BME680_OK)
        return false;
...
int8_t BME680::i2c_read(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) {
    ...
    debug("[0x%X] I2C $%X => ", dev_id >> 1, data[0]);
    ...
    for (uint8_t i = 0; i < len; i++) debug("0x%X ", reg_data[i]);
...
int8_t BME680::i2c_write(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) {
    ...
    for (uint8_t i = 1; i < len + 1; i++) debug("0x%X ", data[i]);
